...
def process_parquet_files_2(parquet_files, filter_string, lob_id, speriod, samples, rps_values,parquet_file_path,Cat):
    ...
    for idx, row in distinct_admins.iterrows():
        ...
        # Calculate cumulative sum of shifted TCE_OEP_1
        shifted_cumsum = dataframe_2['TCE_OEP_1'].shift().cumsum()

        dataframe_2['TCE_OEP_2'] = np.where(
        dataframe_2['RPs'] < 5.0,
        shifted_cumsum * 2.0,
        shifted_cumsum * dataframe_2['RPs']
        )
        ...
        # Calculate cumulative sum of shifted TCE_AEP_1
        shifted_cumsum = dataframe_3['TCE_AEP_1'].shift().cumsum()

        dataframe_3['TCE_AEP_2'] = np.where(
        dataframe_3['RPs'] < 5.0,
        shifted_cumsum * 2.0,
        shifted_cumsum * dataframe_3['RPs']
        )
